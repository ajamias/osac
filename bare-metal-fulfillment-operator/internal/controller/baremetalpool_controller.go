...
func (r *BareMetalPoolReconciler) listAndGroupBareMetalInstances(ctx context.Context, bareMetalPool *v1alpha1.BareMetalPool) (map[string][]*v1alpha1.BareMetalInstance, error) {
	...
	for i := range bareMetalInstanceList.Items {
		...
		// Map back to hostType using metadata annotations for backward compatibility
		hostType := bareMetalInstanceList.Items[i].Annotations["osac.openshift.io/host-type"]
		if hostType == "" {
			log.Error(nil, "BareMetalInstance missing host-type annotation", "name", bareMetalInstanceList.Items[i].Name)
			continue
		}
